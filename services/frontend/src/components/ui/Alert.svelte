...
<div class="rounded-lg border p-4 {colorClasses}" role="alert">
  <div class="flex">
    {#if icon}
      <div class="flex-shrink-0">
        <svg class="h-5 w-5 {iconColorClasses}" viewBox="0 0 20 20" fill="currentColor">
          {@html icons[type]}
        </svg>
      </div>
    {/if}
    ...
    {#if dismissible}
      <div class="ml-auto pl-3">
        <div class="-mx-1.5 -my-1.5">
          <button
            type="button"
            class="inline-flex rounded-md p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-2 hover:bg-black/5 dark:hover:bg-white/5 {iconColorClasses} focus:ring-current"
